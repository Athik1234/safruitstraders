...
class OrderCard extends React.Component {
  ...
  fillDict() {
    let list = this.props.order.content;
    let tempDict = this.state.dict;
    for (let i = 0; i < list.length; i++) {
      if (tempDict[list[i].itemName] === undefined) {
        tempDict[list[i].itemName] = 1;
      } else {
        tempDict[list[i].itemName] = tempDict[list[i].itemName] + 1;
      }
    }
  ...
  orderInfo() {
    axios.get("/api/greengrocer/" + this.props.order.greengrocerId).then(
      response => {
  ...
  render() {
    return this.state.dict && this.state.greengrocer && this.state.customer ? (
      <div>
        <Card>
          <Typography variant="body1" style={{position : "relative", backgroundColor: "#FAFAD2"}} >
            {this.props.userType === "customer" ? 
              <Link to={"/customer/greengrocer/" + this.state.greengrocer.id}>
                <img className="orderCardImage" src= {this.state.greengrocer.information.imageUrl} alt={this.state.greengrocer.information.greengrocerName} />
              </Link> : null
            }
            <i><b>Order from {this.state.greengrocer.information.greengrocerName}</b></i>
          ...
          <Collapse in={this.state.expanded} timeout="auto" unmountOnExit>
            <CardContent>
              <Typography color="textSecondary" paragraph><i><b>From :</b></i> {this.state.greengrocer.address}, {this.state.greengrocer.city}, {this.state.greengrocer.state}, {this.state.greengrocer.zip}</Typography>
              ...
              {this.props.userType === "greengrocer" && this.props.order.endTime !== null && this.props.order.comment !== null ? (
                <Box component="fieldset" mb={3} borderColor="transparent">
                  <Rating name="read-only" value={this.props.order.comment.rating} readOnly />
                  <Typography color="textSecondary" variant="body2"><i>{this.props.order.comment.content}</i></Typography>
                </Box>
